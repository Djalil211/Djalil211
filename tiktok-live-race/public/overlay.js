...
let COUNTRIES = [];
const state = { lanes: [], raceOver: false, last: 0 };
...
/* ---------- بناء المضمار ---------- */
function pickRacers(n){ return shuffle(COUNTRIES.slice()).slice(0,n); }

function buildLanes(){
  elLanes.innerHTML = "";
  state.lanes = [];
  if(!COUNTRIES.length) return;
  pickRacers(6).forEach(c=>{
    ...
    lane.innerHTML = `
      ...
      <div class="racer"><div class="flag">${c.flag}</div><div class="name">${c.name}</div></div>
...
function measure(){ state.lanes.forEach(l=>{ l.maxX = Math.max(60, l.lane.clientWidth - 90); }); }
window.addEventListener("resize",measure);

/* ---------- الدفع ---------- */
function boostByCode(code, amount){
  const l = state.lanes.find(x=>x.c.code===code);
  if(l) applyBoost(l, amount);
}
function boostRandom(amount){
  if(state.lanes.length) applyBoost(pick(state.lanes), amount);
}
...
function spawnHeart(emoji){
  ...
  h.style.left = rnd(0,40)+"px";
  h.style.setProperty("--dx", rnd(-40,40)+"px");
  h.style.animationDuration = rnd(2.6,4.4)+"s";
  h.style.fontSize = rnd(20,32)+"px";
...
function loop(t){
  ...
  state.lanes.forEach(l=>{
    ...
    const crowd = 0.010 + l.hype*0.012;   // زحف بطيء حتى مع قلة الأحداث
...
function finish(winner){
  ...
  confetti();
  // إعادة تشغيل تلقائية بعد 9 ثوانٍ (للبث المستمر)
  setTimeout(()=>{ elWinner.classList.add("hidden"); startRace(); }, 9000);
}
...
document.getElementById("reset").addEventListener("click",startRace);
...
function connect(){
  ...
  ws.onmessage = ev=>{
    ...
    switch(d.type){
      case "countries": COUNTRIES = d.countries; startRace(); requestAnimationFrame(loop); break;
      case "viewers":   elViewers.textContent = (d.count||0).toLocaleString("en-US"); break;
      case "comment":   spawnComment(d.nickname, d.text); break;
      case "like":      spawnHeart("❤️"); break;
      case "join":      spawnComment("🚪", `${d.nickname} انضم`); break;
      case "gift":      spawnGift(d.nickname, d.giftName, d.diamonds, d.repeat); for(let i=0;i<Math.min(d.repeat||1,8);i++) spawnHeart("💎"); break;
      case "boost":     boostByCode(d.code, d.amount); break;
      case "boostRandom": boostRandom(d.amount); break;
      case "streamEnd": elViewers.textContent = "انتهى البث"; break;
    }
  };
  ws.onclose = ()=> setTimeout(connect, 2000); // إعادة الاتصال تلقائياً
  ws.onerror = ()=>{};
}
connect();
